...
function dy = DIRNN(t,x,g,beta)
    ...
    Nt=0;%无噪声
    %Nt=norm(0.1 * ones(10,10));%常数噪声
    %Nt=norm((0.1*sin(t)) * ones(10,10));%时变噪声
    %Nt=norm(randn(10,10));%随机噪声
    dy = [-g*x(1)-beta*x(2)-beta2*x(3)+Nt;%无激活函数
          %-g*AFMSbp(x(1))-beta*x(2)-beta2*x(3)+Nt;%有激活函数
          x(1);
          x(2)
         ];
end
...
function dy = STRNN(t, x, g, beta)
    ...
    dy = [-g * x(1) - beta * x(2) + Nt;%无激活函数
          %-g * AFMSbp(x(1)) - beta * x(2) + Nt;%有激活函数
          sign(x(1));
          0
         ];
end
...
function dXdt = compute_dXdt_nornn(t, X, n5, n4, n3)
    ...
    X = reshape(X, [10, 10]);
    dXdt_matrix_pt = -n3 * Et - Pt_pt - dAt * X +Nt;%无激活函数
    %dXdt_matrix_pt = -n3 * AFMSbp(Et) - Pt_pt - dAt * X +Nt;%有激活函数
    dXdt = dXdt_matrix_pt(:);
end
...
% 用于存储Et的范数 (NORNN)
Et_norms_nornn = zeros(length(t_nornn), 1); % 预分配空间
...
axis(axZoom, [0 3 0 0.3]); % 设置放大后的局部图坐标轴范围
