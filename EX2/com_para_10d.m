...
tspan = [0 10];
% 初始值矩阵
X0 = 0.02 * [1:10; 1:10; 1:10; 1:10; 1:10; 1:10; 1:10; 1:10; 1:10; 1:10];
% 单位矩阵
I = eye(10, 10);
% 参数组
params = [
...
% 定义 At 矩阵
function At = getAt(t)
    coeff = [5 + sin(t), cos(t)./(1:9)];
    At = toeplitz(coeff, coeff'); % 使用 Toeplitz 矩阵构造对称形式
end

% 定义 At 的导数
function dAt = getdAtdt(t)
    coeff = [cos(t), -sin(t)./(1:9)];
    dAt = toeplitz(coeff, coeff');
end
...
legends = {};
for i = 1:size(params, 1)
    plot(t_common, Et_norms_all(i, :), 'LineWidth', 1, 'Color', colors(i, :), 'LineStyle', linestyles{mod(i-1,length(linestyles))+1});
    legends{i} = sprintf('     \\gamma=%3.1f,  \\delta=%3.1f,  \\alpha=%3.1f     ', params(i, :));
end
hold off;
legend(legends, 'Location', 'northeast');%图例在右上角
grid off;
...
hold on;
for i = 1:size(params, 1)
    plot(axZoom, t_common, Et_norms_all(i, :), 'LineWidth', 1, 'Color', colors(i, :), 'LineStyle', linestyles{mod(i-1,length(linestyles))+1});
end
hold off;

axis(axZoom, [0, 2, 0,0.2]);
